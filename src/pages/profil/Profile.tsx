        ...
        <section className="bg-white rounded-[20px] mb-10">
          <div className="grid grid-cols-12 lg:p-[30px] py-[30px] px-[10px] gap-[30px]">
            <div className="md:col-span-3 col-span-12">
              <div className="mb-5">
                <img src={user.userImg || "/images/blank-profile.jpeg"} className='w-full object-cover' alt="Profil Img" />
              </div>
              <div className="lg:hidden flex flex-col items-start gap-[10px] pb-5 md:border-none border-b border-opacity-20 border-solid border-main-darkBase">
                <button type='button' className='flex justify-center items-center gap-[10px] bg-[#f4f4f8] rounded py-[14px] w-full'>
                  ...
                  <span className='font-normal text-xs leading-[15px] text-main-dark'>{t("profile.profileBody.pb_settings")}</span>
                </button>
                <button type='button' className='flex justify-center items-center gap-[10px] bg-[#f4f4f8] rounded py-[14px] w-full'>
                  <img src={repeat} alt="Repeat Svg" />
                  <span className='font-normal text-xs leading-[15px] text-main-dark'>{t("profile.profileBody.pb_refresh_pwd")}</span>
                </button>
              </div>
            </div>
              ...
              <div className="">
                <div className="mb-8">
                  <button type='button' className='lg:flex hidden flex-row items-center gap-2 mb-[15px]'>
                    <img src={repeat} alt="" />
                    <span className='text-main-darkBase font-normal text-xs leading-[15px]'>{t("profile.profileBody.pb_refresh_pwd")}</span>
                  </button>
                  <div className="flex lg:flex-row flex-col justify-between mb-4">
                    <div className="h-fit">
                      <h3 className='text-xl font-normal leading-[25px] text-main-darkBase mb-2'>{user.surname ? user.name + " " + user.surname : user.name}</h3>
                      <span className='text-base font-normal leading-5 text-main-orangePrimary'>{user.position || ""}</span>
                    </div>
                    <div className="flex flex-row items-center justify-between">
                      <p className='text-xl leading-[25px] text-main-darkBase font-normal pr-3 lg:pr-6'>{t("profile.profileBody.pb_person_price")}</p>
                  ...
                  <div className="">
                    <p className='text-sm font-normal leading-[17.5px] text-main-darkBase'>{user.aboutMe || t("profile.blank.about_none")}</p>
                  </div>
                </div>
                    ...
                    <div className="flex flex-row justify-between items-start">
                      <div className="flex flex-col items-start gap-4 justify-start">
                        <div className="flex flex-row gap-2">
                          ...
                          <span className='font-normal text-sm leading-[17.5px] text-main-darkBase'>{user.workAddress || t("profile.blank.workAddress_none")}</span>
                        </div>
                        <div className="flex flex-row gap-2">
                          <img src={house} alt="House Svg" />
                          <span className='font-normal text-sm leading-[17.5px] text-main-darkBase'>{user.nameSystem || t("profile.blank.home_none")}</span>
                        </div>
                        <div className="flex flex-row gap-2">
                          <img src={call} alt="Call Svg" />
                          <span className='font-normal text-sm leading-[17.5px] text-main-darkBase'>{user.phone || t("profile.blank.phone_none")}</span>
                        </div>
                      </div>
                      <div className="lg:flex hidden gap-2 top-0 right-0 items-center">
                        <span className=' text-right font-normal text-sm leading-[17.5px] text-main-darkBase'>{t("profile.profileBody.pb_settings")}</span>
